...
May_S=MAPP(:,5);Jun_S=MAPP(:,6);
Temp_S = [Jan_S, Feb_S, Mar_S, Apr_S, May_S, Jun_S];
...
May_Z=SMDSP(:,5);Jun_Z=SMDSP(:,6);
Temp_Z = [Jan_Z, Feb_Z, Mar_Z, Apr_Z, May_Z, Jun_Z];
...
Jan_T=SMDS(:,1);Feb_T=SMDS(:,3);Mar_T=SMDS(:,5);Apr_T=SMDS(:,7);
May_T=SMDS(:,9);Jun_T=SMDS(:,11);
Temp_T = [Jan_T, Feb_T, Mar_T, Apr_T, May_T, Jun_T];
...
Jun_D=dd;
Temp_D = [Jan_D, Feb_D, Mar_D, Apr_D, May_D, Jun_D];
position_D = 1.45:1:6.45;  % Define position for 12 Month_S boxplots 
box_D = boxplot(Temp_D,'colors','k','positions',position_D,'width',0.13,'sym',' ','whisker',2,'Labels',{'0','4','8','12','16','20'});

% legend('MDS-MAP','MDS-MAP(P)','SMDS','SMDS(P)','GPS');
box_vars=findall(gca,'Tag','Box');
hLegend = legend(box_vars([26,33,23,11,17]), {'GPS','MDS-MAP','MDS-MAP(P)','SMDS-Ny','SMDS(P)-Ny-PM'});

xlabel('3\sigma_{\theta}(\circ)');ylabel('Error(m)');
xlabel('3$\sigma_{\theta}(^\circ)$','interpreter','latex','fontsize',15);% M
